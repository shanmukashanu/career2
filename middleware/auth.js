...
const createSendToken = (user, statusCode, req, res) => {
  ...
  const crossSite = process.env.CROSS_SITE_COOKIES === 'true' || process.env.NODE_ENV === 'production';
  const cookieOptions = {
    expires: new Date(
      Date.now() + (process.env.JWT_COOKIE_EXPIRES_IN || 30) * 24 * 60 * 60 * 1000
    ),
    httpOnly: true,
    // For cross-site requests between different domains, cookies must be SameSite=None and Secure
    secure: crossSite, // Render uses HTTPS in production
